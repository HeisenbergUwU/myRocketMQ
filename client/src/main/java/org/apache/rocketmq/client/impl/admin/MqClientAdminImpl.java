...
public class MqClientAdminImpl implements MqClientAdmin {
    ...
    public CompletableFuture<List<MessageExt>> queryMessage(String address, boolean uniqueKeyFlag, boolean decompressBody, QueryMessageRequestHeader requestHeader, long timeoutMillis) {
        ...
        remotingClient.invoke(address, request, timeoutMillis).thenAccept(response -> { // 阻塞等待前面结果并且消费
            if (response.getCode() == ResponseCode.SUCCESS) {
                List<MessageExt> wrappers = MessageDecoder.decodesBatch(ByteBuffer.wrap(response.getBody()));
    ...
    public CompletableFuture<Map<MessageQueue, Long>> invokeBrokerToResetOffset(String address, ResetOffsetRequestHeader requestHeader, long timeoutMillis) {
        ...
        remotingClient.invoke(address, request, timeoutMillis).thenAccept(response -> {
            if (response.getCode() == ResponseCode.SUCCESS && null != response.getBody()) {
                ...
                log.info("Invoke broker to reset offset success. address:{}, header:{}, offsetTable:{}", address, requestHeader, offsetTable);
            } else {
    ...
    private List<MessageExt> filterMessages(List<MessageExt> messageFoundList, String topic, String key, boolean uniqueKeyFlag) {
        ArrayList<MessageExt> matchedMessages = new ArrayList<>();
        if (uniqueKeyFlag) {
            matchedMessages.addAll(messageFoundList.stream().filter(msg -> {
                return topic.equals(msg.getTopic());
            }).filter(msg -> {
                return key.equals(msg.getMsgId());
